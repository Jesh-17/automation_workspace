...
def dar_individual_keys_datacount():
    """
    ...
    Output:
      - 'dar_individual_keys_datacount.xlsx' (overwritten each run)
    ...
    def prompt_for_file_and_keys() -> Tuple[Path, List[str]]:
        file_in = input("📄 Enter Excel path (or press Enter to use 'dar_refined.xlsx' in current folder): ").strip()
        if file_in:
            xlsx_path = Path(file_in)
        else:
            xlsx_path = Path("dar_refined.xlsx")

        if not xlsx_path.exists():
            raise FileNotFoundError(
                f"Input file not found: {xlsx_path}\n"
                "Provide a valid path or place 'dar_refined.xlsx' in the current directory."
            )
    ...
    def fixed_output_name() -> Path:
        return Path("dar_individual_keys_datacount.xlsx")
